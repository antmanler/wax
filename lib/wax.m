...
static void wax_start_private(char *buf, size_t len, lua_CFunction extensionFunction, va_list extFuncs)
{
    wax_setup();
    ...
    if (extensionFunction) {
        ...
        // Load extentions
        // ---------------
        lua_CFunction extensionFunction;
        //    va_start(extFuncs, len);
        while ((extensionFunction = va_arg(extFuncs, lua_CFunction))) {
            extensionFunction(L);
        }
    }
    ...
    if ([env[@"WAX_TEST"] isEqual:@"YES"]) {
        ...
        exit(1);
    } else if ([env[@"WAX_REPL"] isEqual:@"YES"]) {
        ...
        // Load app
        if (luaL_loadbuffer(L, buf, len, "loading app script") || lua_pcall(L, 0, LUA_MULTRET, 0)) {
            fprintf(stderr, "Error opening app scripts: %s\n", lua_tostring(L, -1));
        }
...
void wax_start(char *initScript, lua_CFunction extensionFunction, ...)
{
    ...
    wax_start_private(appLoadString, strlen(appLoadString), extensionFunction, ap);
    va_end(ap);
...
//        va_list ap;
//        va_start(ap, extensionFunction);
//        while ((extensionFunction = va_arg(ap, lua_CFunction))) {
//            extensionFunction(L);
//        }
//
//        va_end(ap);
...
void wax_start_from_buffer(char *scriptBuffer, size_t size, lua_CFunction extensionFunction, ...)
{
    va_list ap;
    va_start(ap, extensionFunction);
    wax_start_private(scriptBuffer, size, extensionFunction, ap);
    va_end(ap);
}
...
void wax_startWithServer()
{
    ...
    if (!WaxServer) {
        [NSException raise:@"Wax Server Error" format:@"Could load Wax Server"];
    }
...
//    lua_setglobal(L, "exitApp");
    ...
    lua_pushstring(L, [NSSearchPathForDirectoriesInDomains (NSDocumentDirectory, NSUserDomainMask, YES)[0] UTF8String]);
    lua_setglobal(L, "NSDocumentDirectory");

    lua_pushstring(L, [NSSearchPathForDirectoriesInDomains (NSLibraryDirectory, NSUserDomainMask, YES)[0] UTF8String]);
    ...
    if (error) {
        wax_log(WAX_LOG_DEBUG, @"Error creating cache path. %@", [error localizedDescription]);
    }
}
