...
{
    NSString    *searchPath = @luaL_checkstring(L, 1);
...
{
    NSString *path = @luaL_checkstring(L, 1);
...
static int attributes(lua_State *L)
{
    ...
    attributes[@"modifiedAt"] = @(modifiedAt);

    NSTimeInterval createdAt = [attributes[NSFileCreationDate] timeIntervalSince1970];
...
static int createDir(lua_State *L)
{
    ...
    if (lua_gettop(L) > 1)
    {
        createParentDirs = lua_toboolean(L, 2);
    }
