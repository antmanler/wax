...
wax_instance_userdata *wax_instance_create(lua_State *L, id instance, BOOL isClass)
{
    ...
    if (lua_isnil(L, -1)) {
        //wax_log(WAX_LOG_GC, @"Creating %@ for %@(%p)", isClass ? @"class" : @"instance", [instance class], instance);
        lua_pop(L, 1);  // pop nil stack
    } else {
        //wax_log(WAX_LOG_GC, @"Found existing userdata %@ for %@(%p)", isClass ? @"class" : @"instance", [instance class], instance);
    ...
    if (!isClass) {
        //wax_log(WAX_LOG_GC, @"Retaining %@ for %@(%p -> %p)", isClass ? @"class" : @"instance", [instance class], instance, instanceUserdata);
    ...
    //wax_log(WAX_LOG_GC, @"Storing reference of %@ to userdata table %@(%p -> %p)", isClass ? @"class" : @"instance", [instance class], instance, instanceUserdata);
    ...
    //wax_log(WAX_LOG_GC, @"Storing reference to strong userdata table %@(%p -> %p)", [instance class], instance, instanceUserdata);
...
static int __gc(lua_State *L)
{
    ...
    //wax_log(WAX_LOG_GC, @"Releasing %@ %@(%p)", instanceUserdata->isClass ? @"Class" : @"Instance", [instanceUserdata->instance class], instanceUserdata->instance);
        ...
        while (!returnType && [currentClass superclass] != [currentClass class]) { // Walk up the object heirarchy
            ...
            for (int i = 0; !returnType && i < count; i++) {
                ...
                for (int j = 0; !returnType && j < 2; j++) {
                    ...
                    if (!selector) {
                        continue;   // There may be only one acceptable selector sent back
                    }

                    m_description = protocol_getMethodDescription(protocol, selector, YES, YES);

                    if (!m_description.name) {
                        m_description = protocol_getMethodDescription(protocol, selector, NO, YES); // Check if it is not a "required" method
                    }
