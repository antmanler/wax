...
void wax_printTable(lua_State *L, int t)
{
    ...
    if (t < 0) {
        t = lua_gettop(L) + t + 1;  // if t is negative, we need to normalize
    }
...
int wax_fromObjc(lua_State *L, const char *typeDescription, void *buffer)
{
    ...
    case WAX_TYPE_CHAR:
    {
        ...
        if (c <= 1) {
            ...
            // wired on iOS 7 BOOL occupy 4 bytes
            size = 4;
        } else {
            lua_pushinteger(L, c);
        }
    ...
    if (instance) {
        if ([instance isKindOfClass:[NSString class]]) {
            ...
            for (id key in instance) {
                ...
                wax_fromInstance(L, instance[key]);
                lua_rawset(L, -3);
            }
...
void wax_copyToObjc(lua_State *L, const char *typeDescription, int stackIndex, int *outsize, void **outBuffer)
{
    ...
    case WAX_TYPE_ID:
    {
        *outsize = sizeof(id);
        ...
        case LUA_TNUMBER:
            instance = @(lua_tonumber(L, stackIndex));
            break;

        case LUA_TSTRING:
            instance = @(lua_tostring(L, stackIndex));
            break;
...
BOOL wax_selectorForInstance(wax_instance_userdata *instanceUserdata, SEL *foundSelectors, const char *methodName, BOOL forceInstanceCheck)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (!selector) {
            continue;   // There may be only one acceptable selector (i.e. methods with multiple keyword args)
        }
        ...
        if (instanceUserdata->isClass && (forceInstanceCheck || wax_isInitMethod(methodName))) {
            ...
            if ([instanceUserdata->instance methodSignatureForSelector:selector]) {
                addSelector = YES;
            }
        }
...
BOOL wax_isInitMethod(const char *methodName)
{
    if (strncmp(methodName, "init", 4) == 0) {
        if (methodName[4] == '\0') {
            return YES; // It's just an init
        }

        if (isupper(methodName[4]) || isdigit(methodName[4])) {
            return YES; // It's init[A-Z0-9]
        }
    }
            ...
            case WAX_TYPE_STRUCT:
            {
                ...
                for (; openCurlies > 1 || (in[in_index] != WAX_TYPE_UNION_END && in[in_index] != WAX_TYPE_STRUCT_END); in_index++) {
                    if ((in[in_index] == WAX_TYPE_UNION) || (in[in_index] == WAX_TYPE_STRUCT)) {
                        openCurlies++;
                    } else if ((in[in_index] == WAX_TYPE_UNION_END) || (in[in_index] == WAX_TYPE_STRUCT_END)) {
                        openCurlies--;
                    }
                }
